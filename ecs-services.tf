resource "aws_ecs_service" "client" {
  ...
  network_configuration {
    ...
    # defaults to the default VPC security group which allows 
    # all traffic from itself and all outbound traffic
...
resource "aws_ecs_service" "fruits" {
  ...
  network_configuration {
    subnets          = aws_subnet.private.*.id
    assign_public_ip = false
    # defaults to the default VPC security group which allows all traffic 
    # from itself and all outbound traffic
    # instead, we define our own for each ECS service!
...
resource "aws_ecs_service" "vegetables" {
  ...
  network_configuration {
    ...
    assign_public_ip = false
    # defaults to the default VPC security group which allows all traffic 
    # from itself and all outbound traffic instead, we define our own for 
    # each ECS service!
